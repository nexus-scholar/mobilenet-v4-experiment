This failure is a significant scientific finding. You have demonstrated that **Unsupervised Domain Adaptation (TENT)** is insufficient for the "Lab-to-Field" gap in tomato pathology.

### **Analysis of Phase 2 Failure**

The -7% drop confirms that the domain shift is **Semantic**, not just **Statistical**.

* **Statistical Shift:** Lighting, contrast, or color shifts (e.g., "sunny vs. cloudy"). TTA fixes this well.
* **Semantic Shift:** The *features* themselves are different. A "lesion" in the lab looks like a distinct dark spot on gray. A "lesion" in the field looks like a dark spot on a leaf, surrounded by dirt, shadows, and other leaves.
* **Conclusion:** Adjusting Batch Norm statistics (TTA) simply aligns the "dirt" and "shadows" to the "gray background" distribution, confusing the model further. You need **Gradients from Labels** to force the convolutional filters to learn "Field Lesions."

---

### **Phase 3 Strategy: Active Learning (The "Human-in-the-Loop")**

We pivot to the "Efficient Fine-Tuning" hypothesis. We will prove that labeling just **50-100 "Hard" images** is more valuable than labeling 1,000 random ones.

**The Experiment:**

1. **Pool:** Start with the Unlabeled Target Train Set (Simulated "Field Data").
2. **Strategy A (Baseline):** Select  images **Randomly**.
3. **Strategy B (Active):** Select  images with **Highest Entropy** (Uncertainty).
4. **Loop:** Train for a few epochs  Evaluate  Repeat.

**Goal:** Generate a chart showing Strategy B's accuracy rocketing up faster than Strategy A.

### **Execution: The Active Learning Script**

Create `src/active_learning.py`. This script simulates the "Human Loop" using your existing CSV labels as the "Oracle."
...
def run_strategy(strategy_name, full_dataset, test_loader, device, initial_model_state):
    ...
    for r in range(1, ROUNDS + 1):
        ...
        if strategy_name == "Random":
            selected = np.random.choice(pool_indices, size=BUDGET_PER_ROUND, replace=False)
            ...
            top_k_relative = np.argsort(uncertainties)[-BUDGET_PER_ROUND:]
...
def main():
    ...
    # Assuming loaders['target_train'] exists.
    if 'target_train' not in loaders:
        print("Error: Loader did not return 'target_train'. Using a random split of 'target_test' as pool?")
        # Fallback logic if needed, but per report you have 2029 target samples
        # Ideally split 50/50 for "Pool" and "Holdout Test"
        full_target = loaders['target_test'].dataset
        total_size = len(full_target)
        train_size = int(0.5 * total_size)
        test_size = total_size - train_size
        target_pool_set, target_test_set = torch.utils.data.random_split(full_target, [train_size, test_size])
        
        pool_dataset = target_pool_set
        test_loader = DataLoader(target_test_set, batch_size=32, shuffle=False)
    else:
...
### **Why this fixes the "Ghost Class" Problem**

In Phase 2 (TTA), the model guessed "Spider Mites" and reinforced that guess.
In Phase 3 (AL), when the model guesses "Spider Mites" (high entropy), the active learner selects that image. The **Oracle (Ground Truth)** says: *"No, this is Early Blight."* The gradient descent then explicitly punishes the "Spider Mites" neuron and boosts "Early Blight." This will rapidly extinguish the ghost classes.

**Next Step:**
Run the experiment:

```bash
python src/active_learning.py --checkpoint models/mobilenetv4_tomato_domain_shift_best_model.pth

```

This will generate the comparative graph for your thesis.